...
echo("MCMC Diagnostics for Ag.model.01")
...
Ag.gof.02.model <- gof(Ag.mode.02~degree+esp+distance) #Determines model for all other patterns that may emerge
plot(Ag.gof.02.model) #Plots the results from the global GOF model
echo("MCMC Diagnostics for Ag.model.02")
...
Ag.gof.03 <- gof(Amodel.03~model) #Generates a GOF model for specification 3
plot(Ag.gof.03) #Generates a plot for the GOF model 3
Ag.gof.03.global <- gof(Amodel.03~esp+distance) #Determines model for all other patterns. Degree cannot be used for directed sets
plot(Ag.gof.03.global) #Plots the GOF model for all other potential variables besides mdoel
echo("MCMC Diagnostics for Ag.model.03")
